...
| **P17** | MotherDuck | **location-enrichment (ASOF) match quality** | Over one day: `SELECT count(*), count(g.GpsDateTime), median/quantile_cont(0.95)/max of epoch(ev.ts − g.GpsDateTime) FROM <silver status/fault day> ev ASOF LEFT JOIN silver.planet_gps_pings g ON ev.DeviceId = g.DeviceId AND ev.ts >= g.GpsDateTime` | high match % + seconds-scale median gap = enrichment viable ([`MEDALLION_LOADING.md`](MEDALLION_LOADING.md) §Gold ASOF); a big `max` is a parked/offline device (last-known position); unmatched rows = events before the GPS history starts |
| **P18** | MotherDuck | **warehouse health, one query** | The `warehouse_health` union query ([`INCREMENTAL_BACKFILL.md`](INCREMENTAL_BACKFILL.md) §Health): per fact table `count(*)`, distinct devices, `max(event_time)`, hours behind now, cadence tag, + last `warehouse_ingest_log` row per table | `continuous` tables: hours-behind ≈ time since last run; `event` tables: judge by recent-window counts, not staleness; **`latest_event` advancing with no matching log row = an unlogged load** |
...
| 12 | 🔴 | Injects unrequested predicates (partition guards; metadata join type **and fan-out**) | Widened guards seen: `DATE(GpsDateTime) >= DATE('2026-06-27')` on a `>= 06-28` ask; `BETWEEN '06-26' AND '06-30'` around `[06-27,06-29)`. Metadata join `LEFT` one day, inner `JOIN` the next (inner drops devices w/o metadata). **Fan-out (2026-06-30, Demo_fh_vegas4):** `DeviceName` join `ON DeviceId` alone × devices with 2 metadata rows → **exactly 2×** — GPS 350,760→175,378 distinct `(DeviceId,GpsDateTime)`, exc 2,356→1,178 distinct `EventId`; StatusData's `… BETWEEN Device_ActiveFrom AND Device_ActiveTo` join did **not** fan out (550,909=550,909) | ledger 2026-06-29 (forward incremental); ledger 2026-06-30 (vegas daily update); run archive |
...
| 17 | 🟡 | `invalid_value` 400s — transient; plain English **not** immune | 2026-06-29: every 400 was SQL-augmented; plain English didn't fail *that day*. **2026-06-30 (Demo_fh_vegas4): a *bounded* plain-English trips prompt ("…after X and before Y") 400'd 4× while the open-ended form ("…after X") succeeded at once** → retry; prefer open-ended over bounded | run archive 2026-06-29 (5 paired tests); ledger 2026-06-30 (vegas daily update) |
...
| 2026-06-30 | Demo_fh_vegas4 | **metadata-join fan-out (quirk #12)** | GPS & exceptions joined `LatestVehicleMetadata` `ON DeviceId` only → **exactly 2×** rows (each device had 2 metadata rows): gps 350,760→175,378 distinct `(DeviceId,GpsDateTime)`; exc 2,356→1,178 distinct `EventId`. StatusData's join carried `… AND StatusDateTime BETWEEN Device_ActiveFrom AND Device_ActiveTo` → **no** fan-out (550,909=550,909) | same chats as above | silver natural-key dedup absorbs it; shape-check expects `csv_rows` **≥ distinct keys** (not an exact integer multiple — per-device fan-out can differ; that day every device happened to have 2 rows). ACE_TO_CSV #6b/#12 |
...
| 2026-07-02 | MCP server (no DB) | **capability: `GetFeed`** | **Absent, three ways:** tool list (20 tools: `Get`/`GetCountOf`/`GetAceResults`/`GetEntity`/`ListEntities`/`Add`/`Set`/`Remove` + media/EV/HOS helpers) has no feed tool; `Get` schema has only `database`/`typeName`/`search`/`propertySelector`/`resultsLimit`/`sort`/`server` — no `fromVersion`, no method passthrough; sole MCP resource is `mygeotab://entities` | Geotab MCP introspection | why the skill watermarks instead of feed-cursoring — CHANNELS §Relationship to the adapter. Re-check when the MCP server updates |
| 2026-07-02 | demo_fh4 + Demo_fh_vegas4 | entity presence (`GetCountOf`, catalog expansion) | demo_fh4: **DriverChange 9,735 · Audit 35,902 · FuelUsed 33,408 · Controller 594 · FailureMode 283**; DVIRLog/DutyStatusLog/FuelTransaction/ChargeEvent all **0**. vegas: DriverChange 6, DVIRLog 0. **`AuditLog` → `Unsupported entity type`** (typeName is `Audit`); error listed the 52 supported types | Get API | new ENTITY_CATALOG rows + the `Audit`-not-`AuditLog` gotcha; empty-on-demo entities marked unvalidated |
| 2026-07-02 | Demo_fh_vegas4 | **P17** (status→GPS ASOF enrichment) | Day 2026-06-15: **822,203** status events, **100% matched** a same-device prior ping; gap median **2 s**, p95 **23 s**, max **1,236 s**; ran in seconds against 23.7M-row status / 7.86M-row GPS silver | MotherDuck (read-only) | validates MEDALLION §Gold ASOF pattern; max-gap tail = parked/offline devices (last-known position) |
| 2026-07-02 | Demo_fh_vegas4 | **P18** (warehouse health) | One query, 4 fact tables: status 23.7M rows/50 dev, latest `06-30 16:04` (55.5 h behind — loop not run since 06-30); trips 61,760, exceptions 20,473 (event cadence); **GPS latest `07-02 17:20` (6.3 h) but last logged load `06-30 16:04` → an UNLOGGED GPS load happened** — exactly the drift the view exists to surface | MotherDuck (read-only) | validates INCREMENTAL_BACKFILL §Health view; the unlogged load pre-dates this session (read-only run) — whoever loads next should log it |

| 2026-07-02 | Demo_fh_vegas4 | **unlogged-load root cause** (follow-up to P18) | Bronze `gps_raw` has batch **`ace:dsn1wQaKsAhZN1mBPVBP`** — **522,571 rows, `_loaded_at 2026-07-02 17:22`**, silver derived (max `17:20`), **no `warehouse_ingest_log` row**; and the run's other three fact tables were never loaded (newest status/trips/exceptions bronze batches all still 06-30 ~16:0x, matching their logged loads). Signature = a run that finished GPS bronze+silver, then **died mid-loop before logging and before the remaining tables** | MotherDuck (read-only) | Hardening added: **log-ahead** (`started` row at Ace-return persisting window + `gs://` path, `completed` after verify, both plain INSERTs), orphan sweep on session start, and a **log-repair** query reconstructing missing rows from bronze provenance. INCREMENTAL_BACKFILL §state table + §A; SKILL rule #11. The vegas repair itself (INSERT the reconstructed row) is left for the next *write* session |

| 2026-07-03 | Demo_fh_vegas4 | **log rebuild-from-bronze + consolidation** (write session, Claude Code) | `main.warehouse_ingest_log` had 5 rows (all `batch_id` NULL) while bronze held **15** distinct batches → rebuilt all 15 `completed` rows from bronze provenance (watermarks from parsed min/max event, `rows_loaded` from count, `loaded_at` from `_loaded_at`); counts reconciled **exactly** to bronze (gps 12,758,537 · status 23,704,191 · trips 63,407 · exc 21,651). Dropped the legacy `silver.warehouse_ingest_log` mirror (its 5 rows are a subset of the rebuilt main) | MotherDuck | validates §self-healing "the log is reconstructible from bronze"; repairs the 2026-07-02 unlogged `ace:dsn1wQaKsAhZN1mBPVBP` batch. **`main` is now the sole log; the silver mirror is gone** (MAINTAINING ref-DB note + INCREMENTAL_BACKFILL health note updated) |
| 2026-07-03 | Demo_fh_vegas4 | **health view created** | `main.warehouse_health` (`CREATE OR REPLACE VIEW`, INCREMENTAL_BACKFILL §Health) reading `main.warehouse_ingest_log WHERE coalesce(status,'completed')='completed'` | MotherDuck | first run surfaced all 4 tables' freshness; the GPS red flag (unlogged load) cleared once the rebuilt log captured that batch |
| 2026-07-03 | Demo_fh_vegas4 | **four-table forward catch-up** | To ~now (`2026-07-03 05:30`): gps +88,123 net (176,248 CSV, **2× #12 fan-out**, chat `FDuibdz6r8JMHnqxwKAq`); exceptions +1,816 (1,817 CSV, chat `xISI6O1QHfRD2csuO7Sz`); status +2,104,025 net (**4.39M CSV over 5×12h chunks**, 2× fan-out, chats `mAXL8UgV…`/`pBk2et0v…`/`dFK5ch72…`/`S76AkDWy…`/`xo4EcUEr…`); trips via op-D below | objs `prod-us` | log-ahead `started`/`completed` per batch; **0 orphaned started** after; quality battery all 0; every table 50 devices |
| 2026-07-03 | Demo_fh_vegas4 | **status pull TIMEOUT → chunk + bounded subagents** | One open-ended status pull (~1.9M rows / 57 h behind) **timed out at the MCP layer** twice; a single subagent attempting all ~10 windows died on an Anthropic-gateway `ConnectionRefused` at 7.6 min. Fix: **12 h windows (~822K CSV rows each incl. 2× fan-out) via short-lived subagents** (bronze-only + log-ahead `started`; main session derives silver + writes `completed`). All 5 landed; W4 needed 1 retry | Ace + MotherDuck | concrete MCP-timeout data point behind INCREMENTAL_BACKFILL §A "chunk if huge"; **bound each subagent's runtime** (~2 windows) so its own model connection doesn't drop mid-loop |
| 2026-07-03 | Demo_fh_vegas4 | **P16 — trip re-split reconcile (op D) re-validated** | L=24 h pull (`ace:tzMxL3rLOxN6pIcg2T2F`, 7,168 trips, 50 dev, `FROM Trip`, inner active-window metadata join → **no fan-out**). Before: silver-in-window 2,138 → **50 orphans + 5,080 missing** (5,080 = boundary re-splits + 2.4-day forward catch-up + late completions). Guard passed (truth 7,168 > 0, full fleet). After delete + anti-join: **7,168 == 7,168, 0 orphans / 0 missing**; `silver.trips` 61,760 → **66,790** | obj `…/d086b732…` | op D validated a 2nd time on a wider (2.4-day) window; orphans still boundary-clustered at exactly 50 |
| 2026-07-03 | Demo_fh_vegas4 | **quirk #17 recurrence** | Trips (3 tries) and exceptions (2 tries) both hit `invalid_value` 400 on first attempts, cleared on retry; the bounded status chunks did **not** 400 this session | Ace | reinforces #17 (transient, retry) — no doc change |
...
| 2026-07-10 | shared run | **field test: quirk #6 doubling is per-table, not universal** | GPS/status/exceptions each ~2× (3,777,154→1,888,573; 11,956,284→5,978,125; 10,308→5,153); **trips did NOT double** — dedup collapsed all correctly | [shared conversation](https://claude.ai/share/7bafaf73-8018-4312-a16b-5052cca6ef77) | reinforces #6 "not every batch"; **don't assume a fleet-wide halving** — the multiplier is per-table/per-pull |
| 2026-07-10 | shared run | **field test: quirk #2 shards track export size, not row count** | 11.9M-row status → **8** shards; 3.7M-row GPS → **not** sharded | same conv. | shard count ≈ raw export bytes, not a fixed row threshold — reinforces #2 |
| 2026-07-10 | shared run | **field test: quirk #7 column-name drift confirmed** | requested `DiagnosticId` for exception_events; Ace returned it as **`Diagnostic`** | same conv. | reinforces #7 — read the returned `columns` array and map by name; never assume requested==returned |
| 2026-07-10 | shared run | **field test: quirk #17 reproduced** | a tightly-worded, column-bounded trips prompt failed `invalid_value`; the **identical request loosened to open-ended** succeeded immediately on retry | same conv. | reinforces #17 — transient; retry and prefer open-ended over bounded |
| 2026-07-10 | shared run | **field test: P16 reconcile well-calibrated** | **50 orphans** (matches the skill's worked-example 50 exactly); missing 24 (vs 51 in the example — window/fleet dependent) | same conv. | reconcile logic reproduces on a different fleet; orphan count landing on 50 again is reassuring, not a fluke |
...
### Run 2026-07-10 (documented run — public shared conversation)

**What this is:** a full run of the skill replicating a multi-million-row fleet end-to-end, with the
...
Run on a larger fleet than the reference mirrors — corroboration that the documented quirks and
recovery patterns hold there too. Notes from the run, mapped to the catalog:
...
5. **Trip reconcile is well-calibrated (P16).** Their run found **50 orphans** — matching the skill's
   worked example exactly (missing differed: 24 vs 51, which is window/fleet dependent). Reproduced on
   a larger fleet, not a fluke.
6. **Two rough edges (→ IMPROVEMENT_PLAN follow-ups):**
   - **8 stale `started` rows** from a prior crashed session (2026-07-03) sitting in
     ...
     (INCREMENTAL_BACKFILL §The state table) but the run didn't surface/apply it → **discoverability**,
