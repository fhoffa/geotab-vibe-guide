...
12. **Isolate each Geotab source — one schema per source (default), or one database per source.** Geotab entity IDs (`b1`,`b2`,…) are unique only *within* a database, so loading a second source into the *same* tables **collides** in silver/dims (it appends+dedups, not overwrites — worse). Default: a **schema per source** (`geotab.demo_fh4.*`, `geotab.acme_prod.*`) — one connection, easy cross-fleet joins. Escalate to a **database per source** when you need per-tenant **sharing** (Shares are DB-level), **retention/backup**, or **access** boundaries. Never mix sources in one schema; keep `_source_db` in bronze for provenance. ([`MEDALLION_LOADING.md`](references/MEDALLION_LOADING.md) §isolate.)
